...
import { ITaskModel, TaskStatusEnum } from '@common/schemas/mongoose/task';
...
export class UserTasksService {
    ...
    const matchQuery: PipelineStage[] = [
      {
        $match: {
          ...
          $and: [
            ...
            {
              $or: [{ issuedTo: { $in: [new Types.ObjectId(userId)] } }, { issuedBy: new Types.ObjectId(userId) }],
            },
          ],
        },
      },
  ...
  private ensureUserCanUpdateStatus(user: any, task: any) {
    if (user.role?.name?.en === UserRoleEnum.USER) {
      const isAssigned = task.issuedTo?.some((issuedId: any) => issuedId.toString() === user._id.toString());
      if (!isAssigned) {
        throw new UnauthorizedException(errorManager.UNAUTHORIZED);
      }
    }
