...
import { UserJwtPersona, ServiceProviderJwtPersona } from '@common/interfaces/jwt-persona';
...
export class CustomLoggerService extends ConsoleLogger {
  ...
  constructor(@Inject(INQUIRER) private parentClass: object, private appConfig: AppConfig) {
    ...
    const appName = `task-${this.appConfig.NODE_ENV}-backend`;
  ...
  getPersonaWithRole(persona: UserJwtPersona | UserJwtPersona | ServiceProviderJwtPersona) {
    const role = this.getRole(persona);

    if (this.isPersonaUser(persona)) {
      return `${role} ${persona.email}`;
    } else if (this.isPersonaUser(persona)) {
  ...
  getRole(persona: UserJwtPersona | UserJwtPersona | ServiceProviderJwtPersona) {
    if (this.isPersonaUser(persona)) {
